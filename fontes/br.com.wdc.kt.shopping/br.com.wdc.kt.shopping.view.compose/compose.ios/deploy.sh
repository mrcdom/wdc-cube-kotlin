#!/usr/bin/env bash
#
# deploy.sh — Build and deploy the Shopping Compose iOS app
#
# Usage:
...
#     --simulator-name <name>     Simulator name (default: interactive selection)
...
IOS_APP_DIR="$FONTES_DIR/../iosApp"
BUILD_DIR="$IOS_APP_DIR/build/xcode"
SCHEME="ShoppingiOS"
BUNDLE_ID="br.com.wdc.shopping.ios"
...
build_kotlin_framework() {
    ...
    info "Building Kotlin/Native Compose framework..."
    ...
    if [[ "$CLEAN_BUILD" == true ]]; then
        info "Cleaning..."
        ./gradlew ":view-compose-ios:clean" --no-daemon
    fi

    ./gradlew ":view-compose-ios:$gradle_task" --no-daemon

    cd "$SCRIPT_DIR"

    ok "Kotlin Compose framework built successfully"
}
...
deploy_device() {
    ...
    if command -v ios-deploy >/dev/null 2>&1; then
        ...
        open "$IOS_APP_DIR/$SCHEME.xcodeproj"
...
echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║   Shopping Compose iOS — Deploy                 ║"
echo "╠══════════════════════════════════════════════════╣"
echo "║  Target:       $TARGET"
